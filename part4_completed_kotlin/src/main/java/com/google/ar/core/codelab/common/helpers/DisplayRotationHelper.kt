...
class DisplayRotationHelper: DisplayListener {
   ...
   */
  fun displayRotationHelper(context: Context) {
  ...
  fun getCameraSensorRelativeViewportAspectRatio(cameraId: String): Float {
    ...
    when (cameraSensorToDisplayRotation) {
      270 -> {
        aspectRatio = viewportHeight.toFloat() / viewportWidth.toFloat()
      }
      180 -> {
      ...
      0 -> {
        ...
        throw RuntimeException("Unhandled rotation: $cameraSensorToDisplayRotation")
      }
    }
    return aspectRatio;
  }
  ...
  fun toDegrees(rotation: Int): Int {
